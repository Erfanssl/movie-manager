...
class Worst10 extends Component {
    constructor(props) {
        super(props);
        this.state = {
            db: []
    ...
    componentDidMount() {
        ...
        const table = document.querySelector('.worst10__table');
    ...
    render() {
        return (
            <div className="container mt-5">
                ...
                <table className="table worst10__table text-white bg-info d-none">
                    <thead>
                    <tr>
                        <th>Title</th>
                        <th>My Rate</th>
                    ...
                    <tbody>
                    { this.state.db.sort((a, b) => a.myRating - b.myRating).slice(0, 10).map((item, index) => (
                        <MovieItem
                            key={ item.id }
                            id={ item.id }
