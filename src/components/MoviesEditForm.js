...
class MoviesEditForm extends Component {
    ...
    componentDidMount() {
        ...
        if (data.db) {
            ...
            const db = data.db.find(item => item.id === id);
            this.setState(() => db);
            ...
            if (db.checked) {
                cinematographyText.classList.add('d-none');
                cinematographyContainer.classList.add('d-none');
                animationContainer.classList.remove('d-none');
                animationText.classList.remove('d-none');
            } else {
                animationContainer.classList.add('d-none');
                animationText.classList.add('d-none');
                cinematographyText.classList.remove('d-inline-block');
                cinematographyContainer.classList.remove('d-inline-block');
            }
        }
    }
    ...
    handleForm(e) {
        ...
        if (data.db) {
            ...
            for (let i = 0; i < data.db.length; i++) {
                if (data.db[i].id === id) data.db[i] = this.state;
            }
            data.db.sort((a,b) => b.myRating - a.myRating);
                ...
                <div className="form-group bg-glass py-2 px-4 mt-2 container">
                    <div className="mid">
                        <label className="rocker">
                            <input onChange={ this.handleCheckbox } checked={ this.state.checked } type="checkbox"/>
                    ...
                    <button className="btn btn-success form-control mt-3 movies__submit py-1 pb-3" disabled>Edit</button>
                </div>
            </form>
        );
    }
